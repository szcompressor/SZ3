...
void runFunctionalTest() {
    ...
    // decompress() reports the size here; initialise it so the value passed in is never garbage.
...
TEST(SZ3_LosslessTest, LosslessZstd) { runAllTest<SZ3::Lossless_zstd>(); }
TEST(SZ3_LosslessTest, LosslessBypass) { runAllTest<SZ3::Lossless_bypass>(); }
