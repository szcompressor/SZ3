...
inline void detectSysEndianType() {
    // get sys endian type
    int x_temp = 1;
    char *y_temp = reinterpret_cast<char *>(&x_temp);
...
template <typename T>
void process_data(const SZ3::Config &conf, const char *oriFilePath, int dataEndianType, hid_t fid, hid_t sid,
                  hid_t cpid, const char *datasetName, hid_t h5TypeLE, hid_t h5TypeBE) {
...
int main(int argc, char *argv[]) {
    ...
    std::map<std::string, int> dataTypeMap = {
        {"-f", SZ_FLOAT},    {"-d", SZ_DOUBLE},  {"-i8", SZ_INT8},    {"-u8", SZ_UINT8},  {"-i16", SZ_INT16},
        {"-u16", SZ_UINT16}, {"-i32", SZ_INT32}, {"-u32", SZ_UINT32}, {"-i64", SZ_INT64}, {"-u64", SZ_UINT64}};
    ...
    switch (dataType) {
        case SZ_FLOAT:
            process_data<float>(conf, oriFilePath, dataEndianType, fid, sid, cpid, DATASET, H5T_IEEE_F32LE,
                                H5T_IEEE_F32BE);
            break;
        case SZ_DOUBLE:
            process_data<double>(conf, oriFilePath, dataEndianType, fid, sid, cpid, DATASET, H5T_IEEE_F64LE,
                                 H5T_IEEE_F64BE);
            break;
        case SZ_INT8:
            process_data<int8_t>(conf, oriFilePath, dataEndianType, fid, sid, cpid, DATASET, H5T_STD_I8LE,
                                 H5T_STD_I8BE);
            break;
        case SZ_UINT8:
            process_data<uint8_t>(conf, oriFilePath, dataEndianType, fid, sid, cpid, DATASET, H5T_STD_U8LE,
                                  H5T_STD_U8BE);
            break;
        case SZ_INT16:
            process_data<int16_t>(conf, oriFilePath, dataEndianType, fid, sid, cpid, DATASET, H5T_STD_I16LE,
                                  H5T_STD_I16BE);
            break;
        case SZ_UINT16:
            process_data<uint16_t>(conf, oriFilePath, dataEndianType, fid, sid, cpid, DATASET, H5T_STD_U16LE,
                                   H5T_STD_U16BE);
            break;
        case SZ_INT32:
            process_data<int32_t>(conf, oriFilePath, dataEndianType, fid, sid, cpid, DATASET, H5T_STD_I32LE,
                                  H5T_STD_I32BE);
            break;
        case SZ_UINT32:
            process_data<uint32_t>(conf, oriFilePath, dataEndianType, fid, sid, cpid, DATASET, H5T_STD_U32LE,
                                   H5T_STD_U32BE);
            break;
        case SZ_INT64:
            process_data<int64_t>(conf, oriFilePath, dataEndianType, fid, sid, cpid, DATASET, H5T_STD_I64LE,
                                  H5T_STD_I64BE);
            break;
        case SZ_UINT64:
            process_data<uint64_t>(conf, oriFilePath, dataEndianType, fid, sid, cpid, DATASET, H5T_STD_U64LE,
                                   H5T_STD_U64BE);
            break;
        default:
