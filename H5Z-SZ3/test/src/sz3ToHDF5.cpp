...
int main(int argc, char * argv[])
{
   ...
   // detectSysEndianType();
    ...
    //TODO take in from cmd or config file
    cmp_algo = 1;
    interp_algo = 1;

    //printf("cfgFile=%s\n", cfgFile);
    sprintf(outputFilePath, "%s.sz3.h5", oriFilePath);
...
//		printf("cd_values[%d]=%u\n", i, cd_values[i]);
    ...
    /* create HDF5 file */
    if (0 > (fid = H5Fcreate(outputFilePath, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT))) { printf("Error in H5Pcreate"); exit(0); }

    /*Create dataspace. Setting maximum size */
    if (0 > (sid = H5Screate_simple(dim, dims, NULL))) { printf("Error in H5Screate_simple"); exit(0); }

    /* setup dataset creation properties */
    if (0 > (cpid = H5Pcreate(H5P_DATASET_CREATE))) { printf("Error in H5Pcreate"); exit(0); }

    /* Add the SZ compression filter and set the chunk size */
    if (0 > H5Pset_filter(cpid, H5Z_FILTER_SZ3, H5Z_FLAG_MANDATORY, cd_nelmts, cd_values)) { printf("Error in H5Pcreate"); exit(0); }
	...
	readfile(oriFilePath, nbEle, data);		
        ...
        {
            if (0 > (idsid = H5Dcreate(fid, DATASET, H5T_IEEE_F32LE, sid, H5P_DEFAULT, cpid, H5P_DEFAULT))) { printf("Error in H5Dcreate"); exit(0); }
            if (0 > H5Dwrite(idsid, H5T_IEEE_F32LE, H5S_ALL, H5S_ALL, H5P_DEFAULT, data)) { printf("Error in H5Dwrite"); exit(0); }
        }
	...
	printf("original data = ");
        ...
        if(dataEndianType == LITTLE_ENDIAN_DATA)
        {
            if (0 > (idsid = H5Dcreate(fid, DATASET, H5T_IEEE_F64LE, sid, H5P_DEFAULT, cpid, H5P_DEFAULT))) { printf("Error in H5Dcreate"); exit(0); }
            if (0 > H5Dwrite(idsid, H5T_IEEE_F64LE, H5S_ALL, H5S_ALL, H5P_DEFAULT, data)) { printf("Error in H5Dwrite"); exit(0); }
        }
        else //BIG_ENDIAN_DATA
        {
	...
	printf("original data = ");
        ...
        if(dataEndianType == LITTLE_ENDIAN_DATA)
        {
            if (0 > (idsid = H5Dcreate(fid, DATASET, H5T_STD_I8LE, sid, H5P_DEFAULT, cpid, H5P_DEFAULT))) { printf("Error in H5Dcreate"); exit(0); }
            if (0 > H5Dwrite(idsid, H5T_STD_I8LE, H5S_ALL, H5S_ALL, H5P_DEFAULT, data)) { printf("Error in H5Dwrite"); exit(0); }
        }
        else //BIG_ENDIAN_DATA
        {
	...
	printf("original data = ");
        ...
        if(dataEndianType == LITTLE_ENDIAN_DATA)
        {
            if (0 > (idsid = H5Dcreate(fid, DATASET, H5T_STD_U8LE, sid, H5P_DEFAULT, cpid, H5P_DEFAULT))) { printf("Error in H5Dcreate"); exit(0); }
            if (0 > H5Dwrite(idsid, H5T_STD_U8LE, H5S_ALL, H5S_ALL, H5P_DEFAULT, data)) { printf("Error in H5Dwrite"); exit(0); }
        }
        else //BIG_ENDIAN_DATA
        {
	...
	printf("original data = ");
        ...
        if(dataEndianType == LITTLE_ENDIAN_DATA)
        {
            if (0 > (idsid = H5Dcreate(fid, DATASET, H5T_STD_I16LE, sid, H5P_DEFAULT, cpid, H5P_DEFAULT))) { printf("Error in H5Dcreate"); exit(0); }
            if (0 > H5Dwrite(idsid, H5T_STD_I16LE, H5S_ALL, H5S_ALL, H5P_DEFAULT, data)) { printf("Error in H5Dwrite"); exit(0); }
        }
        else //BIG_ENDIAN_DATA
        {
	...
	printf("original data = ");
        ...
        if(dataEndianType == LITTLE_ENDIAN_DATA)
        {
            if (0 > (idsid = H5Dcreate(fid, DATASET, H5T_STD_U16LE, sid, H5P_DEFAULT, cpid, H5P_DEFAULT))) { printf("Error in H5Dcreate"); exit(0); }
            if (0 > H5Dwrite(idsid, H5T_STD_U16LE, H5S_ALL, H5S_ALL, H5P_DEFAULT, data)) { printf("Error in H5Dwrite"); exit(0); }
        }
        else //BIG_ENDIAN_DATA
        {
    ...
    {
       printf("%i \t %i", sizeof(int), sizeof(int32_t));
       ...
       readfile(oriFilePath, nbEle, data);
        ...
        if(dataEndianType == LITTLE_ENDIAN_DATA)
        {
            if (0 > (idsid = H5Dcreate(fid, DATASET, H5T_STD_I32LE, sid, H5P_DEFAULT, cpid, H5P_DEFAULT))) { printf("Error in H5Dcreate"); exit(0); }
            if (0 > H5Dwrite(idsid, H5T_STD_I32LE, H5S_ALL, H5S_ALL, H5P_DEFAULT, data)) { printf("Error in H5Dwrite"); exit(0); }
        }
        else //BIG_ENDIAN_DATA
        {
	...
	printf("original data = ");
        ...
        if(dataEndianType == LITTLE_ENDIAN_DATA)
        {
            if (0 > (idsid = H5Dcreate(fid, DATASET, H5T_STD_U32LE, sid, H5P_DEFAULT, cpid, H5P_DEFAULT))) { printf("Error in H5Dcreate"); exit(0); }
            if (0 > H5Dwrite(idsid, H5T_STD_U32LE, H5S_ALL, H5S_ALL, H5P_DEFAULT, data)) { printf("Error in H5Dwrite"); exit(0); }
        }
        else //BIG_ENDIAN_DATA
        {
	...
	printf("original data = ");
        ...
        if(dataEndianType == LITTLE_ENDIAN_DATA)
        {
            if (0 > (idsid = H5Dcreate(fid, DATASET, H5T_STD_I64LE, sid, H5P_DEFAULT, cpid, H5P_DEFAULT))) { printf("Error in H5Dcreate"); exit(0); }
            if (0 > H5Dwrite(idsid, H5T_STD_I64LE, H5S_ALL, H5S_ALL, H5P_DEFAULT, data)) { printf("Error in H5Dwrite"); exit(0); }
        }
        else //BIG_ENDIAN_DATA
        {
	...
	printf("original data = ");
        ...
        if(dataEndianType == LITTLE_ENDIAN_DATA)
        {
            if (0 > (idsid = H5Dcreate(fid, DATASET, H5T_STD_U64LE, sid, H5P_DEFAULT, cpid, H5P_DEFAULT))) { printf("Error in H5Dcreate"); exit(0); }
            if (0 > H5Dwrite(idsid, H5T_STD_U64LE, H5S_ALL, H5S_ALL, H5P_DEFAULT, data)) { printf("Error in H5Dwrite"); exit(0); }
        }
        else //BIG_ENDIAN_DATA
        {
    ...
    if (0 > H5Sclose(sid)) { printf("Error in H5Sclose"); exit(0); }
    if (0 > H5Pclose(cpid)) { printf("Error in H5Pclose"); exit(0); }
    if (0 > H5Fclose(fid)) { printf("Error in H5Fclose"); exit(0); }
    free(cd_values);
