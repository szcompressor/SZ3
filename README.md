...
* SZ 3.3.3: decompression is bounds-checked against corrupted input, and several bugs reachable on valid data are fixed. The compressed format is unchanged. `find_package(SZ3)` now works against an installed SZ3; where pkg-config finds no libzstd, SZ3 links the bundled Zstandard v1.4.5 statically instead (see 3rd party libraries below).

## 3rd party libraries/tools
* [Zstandard](https://facebook.github.io/zstd/) v1.4.5 will be fetched if libzstd can not be found by pkg-config, or if `-DSZ3_USE_BUNDLED_ZSTD=ON` is given. It is linked statically as `libsz3_zstd` and kept private to SZ3: it exports no symbols and puts no `zstd.h` on a consumer's include path.
