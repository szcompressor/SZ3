 ...
 */
template <typename T>
void _smawk(const std::vector<size_t> &rows, const std::vector<size_t> &cols,
            const std::function<T(size_t, size_t)> &lookup, std::vector<size_t> *result) {
    // Recursion base case
    ...
    for (size_t col : cols) {
        while (true) {
            ...
            if (lookup(row, col) >= lookup(row, _cols.back())) break;
            _cols.pop_back();
        }
        if (_cols.size() < rows.size()) _cols.push_back(col);
    }
    ...
    for (size_t r = 0; r < rows.size(); r += 2) {
        ...
        if (r < rows.size() - 1) stop = col_idx_lookup[(*result)[rows[r + 1]]];
 ...
 */
template <typename T>
std::vector<size_t> smawk(const size_t num_rows, const size_t num_cols,
...
class Matrix {
   ...
   public:
    ...
    inline T get(size_t i, size_t j) { return data[i * num_cols + j]; }

    inline void set(size_t i, size_t j, T value) { data[i * num_cols + j] = value; }
};

template <class DT>
void cluster(DT *array, size_t n, int &k, size_t *clusters, DT *centroids) {
    // ***************************************************
    ...
    sort(sort_idxs.begin(), sort_idxs.end(), [&array](size_t a, size_t b) { return array[a] < array[b]; });
...
template <class T>
void get_cluster(T *data, size_t num, float &level_start, float &level_offset, int &level_num, size_t sample_num) {
