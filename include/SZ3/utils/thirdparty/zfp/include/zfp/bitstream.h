#ifndef SZ3_ZFP_ZFP_BITSTREAM_H
#define SZ3_ZFP_ZFP_BITSTREAM_H

#include <stddef.h>
#include "internal/zfp/types.h"
#include "internal/zfp/system.h"

/* forward declaration of opaque type */
/* SZ3: declared locally first. This header is included inside a namespace, where the
   elaborated-type-specifier below would otherwise bind to an enclosing-scope `bitstream`
   -- an installed zfp's -- and then collide with the definition. */
struct bitstream;
...
/* SZ3: defined upstream in bitstream.c, which is not vendored. `extern_` is extern "C",
   so this would escape the namespace and bind to an installed libzfp. Unused here. */
