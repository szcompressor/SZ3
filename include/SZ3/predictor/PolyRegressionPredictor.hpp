...
class PolyRegressionPredictor : public concepts::PredictorInterface<T, N> {
   public:
    ...
    //        PolyRegressionPredictor() : quantizer_independent(0), quantizer_liner(0), quantizer_poly(0),
    //        current_coeffs{0} {
    //            init_poly();
    //        }

    PolyRegressionPredictor(uint block_size, T eb)
        : quantizer_independent(eb / 5 / block_size),
    ...
    void precompress_data(const iterator &) const noexcept override{}

    void postcompress_data(const iterator &) const noexcept override{}

    void predecompress_data(const iterator &) const noexcept override{}

    void postdecompress_data(const iterator &) const noexcept override{}

    inline T estimate_error(const iterator &iter) const noexcept override{ return fabs(*iter - predict(iter)); }

    bool precompress_block(const std::shared_ptr<Range> &range) noexcept override{
    ...
    template <uint NN = N>
    inline typename std::enable_if<NN != 1 && NN != 2, std::array<double, M>>::type get_poly_index(
        ...
        *reinterpret_cast<size_t *>(c) = regression_coeff_quant_inds.size();
        ...
        if (!regression_coeff_quant_inds.empty()) {
            ...
            encoder.preprocess_encode(
                ...
                2 * std::max(std::max(quantizer_independent.get_out_range().second, quantizer_liner.get_out_range().second),
                             quantizer_poly.get_out_range().second));
            encoder.save(c);
    ...
    bool predecompress_block(const std::shared_ptr<Range> &range) noexcept override{
   ...
   private:
    ...
    //        std::array<T, M>
    //        compute_regression_coefficients(const std::shared_ptr<Range> &range, const std::array<size_t, N> &dims)
    //        const {
    //            std::array<double, M> sum{0};
    //            {
    //                for (auto iter = range->begin(); iter != range->end(); ++iter) {
    ...
    void pred_and_recover_coefficients() {
        current_coeffs[0] =
            quantizer_independent.recover(current_coeffs[0], regression_coeff_quant_inds[regression_coeff_index++]);
        for (int i = 1; i < N + 1; i++) {
            current_coeffs[i] =
        ...
        for (int i = N + 1; i < M; i++) {
            current_coeffs[i] =
                quantizer_poly.recover(current_coeffs[i], regression_coeff_quant_inds[regression_coeff_index++]);
        }
    }

    void display_coef_aux(std::array<T, M * M> aux) {
        for (int i = 0; i < M; i++) {
            for (int j = 0; j < M; j++) std::cout << std::setw(10) << std::setprecision(6) << aux[i * M + j] << " ";
            std::cout << std::endl;
        }
    }
