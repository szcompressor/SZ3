...
char *SZ_compress_impl_OMP(SZ::Config &conf, const T *data, size_t &outSize) {
...
//            auto minmax = std::minmax_element(data_t.begin(), data_t.end());
            auto minmax = std::minmax_element(data_t, data_t);
...
template<class T, SZ::uint N>
void SZ_decompress_impl_OMP(const SZ::Config &conf, char *cmpData, size_t cmpSize, T *decData) {
...
#ifdef _OPENMP
    if (conf.openmp) {
        return SZ_compress_impl_OMP<T, N>(conf, data, outSize);
    } else {
...
#ifdef _OPENMP
    if (conf.openmp) {
        SZ_decompress_impl_OMP<T, N>(conf, cmpData, cmpSize, decData);
    } else {
