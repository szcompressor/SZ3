                    ...
                    for (size_t i = 0; i < conf.dims[1]; i++) {
                        for (size_t t = 1; t < conf.dims[0]; t++) {
                            ...
                            quant_inds[pred_idx++] = quantizer.quantize_and_overwrite(
                                    data[idx], data[(t - 1) * conf.dims[1] + i]);
                        }
                    }
...
//            timer.stop("Predition & Quantization");
            ...
            auto buffer = (uchar *) malloc(4 * conf.num * sizeof(T));
...
//        T *decompress(uchar const *lossless_compressed_data, const size_t length) {
        T *decompress(const Config &conf, uchar const *cmpData, size_t cmpSize, T *dec_data) {
