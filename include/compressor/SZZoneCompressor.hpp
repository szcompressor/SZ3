...
// type T
    ...
    class SZZoneCompressor {
    public:

        SZZoneCompressor(const Config<T, N> &conf, Predictor predictor, Quantizer quantizer, Encoder encoder, Lossless lossless) :
                predictor(predictor), quantizer(quantizer), encoder(encoder), lossless(lossless), block_size(conf.block_size), stride(conf.stride),
...
//                      << "s" << std::endl;
            ...
            for (int z = 0; z < num_zone + 1; z++) {
                lossless_data[z] = lossless.compress(compressed_data[z],
                                                     compressed_data_pos[z] - compressed_data[z],
                                                     compressed_size[z]);
        ...
        void decompress_zone(uchar const *lossless_compressed_data, const size_t length, T *dec_data) {
...
//            return dec_data.release();
    ...
    SZZoneCompressor<T, N, Predictor, Quantizer, Encoder, Lossless_zstd>
    make_sz_zone_compressor(const Config<T, N> &conf,
                            Predictor predictor,
                            Quantizer quantizer,
                            Encoder encoder
    ) {
        return SZZoneCompressor<T, N, Predictor, Quantizer, Encoder, Lossless_zstd>(conf, predictor, quantizer, encoder, Lossless_zstd());
    }
}
#endif
