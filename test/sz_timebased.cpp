...
float SZ_compress_build_frontend(std::unique_ptr<T[]> const &data, const SZ::Config<T, N> &conf) {
    ...
    if (conf.enable_lorenzo) {
        if (use_single_predictor) {
            return SZ_compress_bulid_backend<T>(data, conf,
                                                make_sz3_timebased_frontend(conf,
                                                                            SZ::LorenzoPredictor<T, N - 1, 1>(conf.eb),
                                                                            quantizer));
        } else {
    ...
    if (conf.enable_2ndlorenzo) {
        if (use_single_predictor) {
            return SZ_compress_bulid_backend<T>(data, conf,
                                                make_sz3_timebased_frontend(conf,
                                                                            SZ::LorenzoPredictor<T, N - 1, 2>(conf.eb),
                                                                            quantizer));
        } else {
    ...
    if (conf.enable_regression) {
        if (use_single_predictor) {
            return SZ_compress_bulid_backend<T>(data, conf,
                                                make_sz3_timebased_frontend(conf, SZ::RegressionPredictor<T, N - 1>(
                                                        conf.block_size,
                                                        conf.eb),
                                                                            quantizer));
        } else {
    ...
    return SZ_compress_bulid_backend<T>(data, conf,
                                        make_sz3_timebased_frontend(conf, SZ::ComposedPredictor<T, N - 1>(predictors),
                                                                    quantizer));
}
