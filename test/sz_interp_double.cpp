...
interp_compress_decompress_nowritefile(char *path, T *data, size_t num, double eb, int interp_level,
    ...
    {
        auto sz_dec = SZ::SZInterpolationCompressor<T, N, SZ::LinearQuantizer<T>, SZ::HuffmanEncoder<int>, SZ::Lossless_zstd>(
                SZ::LinearQuantizer<T>(eb),
                SZ::HuffmanEncoder<int>(),
                SZ::Lossless_zstd(),
                dims,
    ...
    compression_ratio = num * sizeof(T) * 1.0 / compressed_size;
...
template<class T, uint N, class ... Dims>
META::meta_compress_info
interp_compress_decompress(char *path, T *data, size_t num, double eb, int interp_level, int interp_op,
                           int direction_op,
                           int block_size, int interp_block_size, SZ_Option sz_op, Dims ... args) {
    std::string compressed_file_name(path);
    META::meta_compress_info compressInfo;
...
template<class T, uint N, class ... Dims>
double
interp_compress_test_block(T *data, size_t num, double eb, int interp_level, int interp_op, int direction_op,
                           int block_size,
    ...
    SZ::Config<T, N> conf(eb, std::array<size_t, N>{static_cast<size_t>(std::forward<Dims>(args))...});
...
//    auto dims = std::array<size_t, N>{static_cast<size_t>(std::forward<Dims>(args))...};
//    auto sz = SZ::SZBlockInterpalationCompressor<T, N, SZ::LinearQuantizer<T>, SZ::HuffmanEncoder<int>, SZ::Lossless_zstd>(
//            conf,
//            SZ::LinearQuantizer<T>(eb),
//            SZ::HuffmanEncoder<int>(),
//            SZ::Lossless_zstd(),
...
template<class T, uint N, class ... Dims>
double
interp_compress_test(T *data, size_t num, double eb, int interp_level, int interp_op, int direction_op, int block_size,
                     int interp_block_size, SZ_Option sz_op, Dims ... args) {
...
META::meta_compress_info interp_tuning(char *path, double reb, Dims ... args) {
    ...
    for (int i = 0; i < 2; i++) {
        ratio = interp_compress_test<T, N>(data.get(), num, eb, interp_level, i, direction_op, block_size,
    ...
    ratio = interp_compress_test<T, N>(data.get(), num, eb, interp_level, interp_op, 5, block_size, interp_block_size,
                                       sz_op,
                                       args...);
    ...
    return interp_compress_decompress<T, N>(path, data.get(), num, eb, interp_level, interp_op, direction_op, block_size,
                                            interp_block_size, sz_op,
                                            args...);
}
...
void interp_meta_tuning(char *path, double reb, Dims ... args) {
    ...
    std::vector<T> sampling_data(sampling_num, 0);
...
//        }
    ...
    for (int i = 0; i < 2; i++) {
        ratio = interp_compress_test_block<T,N>(sampling_data.data(), sampling_num, eb, interp_level, i, direction_op,
    ...
    ratio = interp_compress_test_block<T, N>(sampling_data.data(), sampling_num, eb, interp_level, interp_op, 5,
                                             block_size,
...
//    printf("tuning capacity: %d\n", capacity);
        ...
        auto result = interp_compress_decompress<T, N>(path, data.get(), num, eb, interp_level, interp_op, direction_op,
                                                       block_size,
...
int main(int argc, char **argv) {
    ...
    if (argp >= argc) {
        if (dim == 1) {
            interp_tuning<double, 1>(argv[1], reb, dims[0]);
        } else if (dim == 2) {
            interp_tuning<double, 2>(argv[1], reb, dims[0], dims[1]);
        } else if (dim == 3) {
            interp_meta_tuning<double, 3>(argv[1], reb, dims[0], dims[1], dims[2]);
        } else if (dim == 4) {
            interp_tuning<double, 4>(argv[1], reb, dims[0], dims[1], dims[2], dims[3]);
        }
        return 0;
    }
    ...
    if (dim == 1) {
        interp_compress_decompress<double, 1>(argv[1], data.get(), num, eb, interp_level, interp_op, direction_op,
                                              block_size,
                                              interp_block_size, sz_op, dims[0]);
    } else if (dim == 2) {
        interp_compress_decompress<double, 2>(argv[1], data.get(), num, eb, interp_level, interp_op, direction_op,
                                              block_size,
                                              interp_block_size, sz_op, dims[0], dims[1]);
    } else if (dim == 3) {
        interp_compress_decompress<double, 3>(argv[1], data.get(), num, eb, interp_level, interp_op, direction_op,
                                              block_size,
                                              interp_block_size, sz_op, dims[0], dims[1], dims[2]);
    } else if (dim == 4) {
        interp_compress_decompress_nowritefile<double, 4>(argv[1], data.release(), num, eb, interp_level, interp_op,
                                                          direction_op,
                                                          block_size,
